...
import {
  ...
  PopoverContent,
  PopoverTrigger,
} from "@/components/ui/Popover-Temp";
import { Calendar } from "@/components/ui/Calendar-Temp";
import { uploadProduct, generateDescriptionApi } from "../lib/api"; 
...
import {
  ...
  SelectTrigger,
  SelectValue,
} from "@/components/ui/Select-Temp";
...
useEffect(() => {
  ...
  console.log("Changed:", e.target.id, e.target.name, e.target.value);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!image || !expiryDate || !location || !formData.category) {
      setError(
        "Please fill all fields, including image, expiry date, category, and allow location."
      );
      return;
    }
    setIsLoading(true);
  ...
  const keywords = (formData.keywords || "").trim();
    console.log("1. 'Generate with AI' button clicked.");
    if (!keywords) {
      console.error("2. Generation failed: No keywords provided.");
      toast.error("Please enter some keywords first.");
      return;
    }

    console.log("3. Keywords found. Attempting to call API with:", keywords);
    setIsGenerating(true);
    try {
      const { description } = await generateDescriptionApi(keywords);
      console.log("4. API call successful. Received description:", description);
setFormData(prev => ({ ...prev, description: description }));      toast.success("Description generated!");
    } catch (err) {
        ...
        <Card className="w-full max-w-2xl text-center">
          ...
          <CardContent>
            <p className="text-gray-600">
              Thank you for registering! Your seller account is currently under
              review by our team. This process usually takes 24-48 hours. We'll
              notify you via email once your account is approved.
            </p>
          </CardContent>
        </Card>
      </div>
        ...
        <CardContent className="p-8">
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <FormSection
              ...
              icon={<Package className="text-brand-green" />}
            >
                ...
                <div className="space-y-2">
                  ...
                  >
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="Packaged Goods">
                        Packaged Goods
                      </SelectItem>
                    </SelectContent>
                  </Select>
              ...
              <div className="space-y-2 pt-4">
                ...
                <Input
                  ...
                  onChange={handleInputChange}
                  placeholder="e.g., 'Fresh bread', 'sourdough', 'baked today'"
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  onChange={handleInputChange}
                  placeholder="A short, appealing description of your item..."
                />
              </div>
            </FormSection>
            ...
            <FormSection
              ...
              icon={<Tag className="text-brand-green" />}
            >
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
            ...
            <FormSection
              title="Image & Expiry"
              icon={<FileImage className="text-brand-green" />}
            >
            ...
            >
              <p className="text-sm text-gray-500">
                Click on the map to set the precise pickup point for your item.
              </p>
            ...
            <div className="pt-6">
              <Button
                type="submit"
                size="lg"
                className="w-full bg-brand-orange hover:bg-brand-orange/90 text-lg"
                disabled={isLoading}
              >
